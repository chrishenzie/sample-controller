...
const (
	...
	// MessageResourceSynced is the message used for an Event fired when a TaskCluster
...
func NewTaskClusterController(
	...
	// Set up an event handler for when TaskCluster resources change
	...
	// owned by a TaskCluster resource then the handler will enqueue that TaskCluster resource for
...
func (c *TaskClusterController) Run(ctx context.Context, workers int) error {
	...
	// Launch two workers to process TaskCluster resources
	for i := 0; i < workers; i++ {
...
func (c *TaskClusterController) syncHandler(ctx context.Context, objectRef cache.ObjectName) error {
	...
	if err != nil {
		// The TaskCluster resource may no longer exist, in which case we stop
		// processing.
	...
	// Get the deployment with the name specified in TaskCluster.spec
	deployment, err := c.deploymentsLister.Deployments(taskCluster.Namespace).Get(deploymentName)
	// If the resource doesn't exist, we'll create it
	if errors.IsNotFound(err) {
		deployment, err = c.kubeclientset.AppsV1().Deployments(taskCluster.Namespace).Create(context.TODO(), newDeployment(taskCluster), metav1.CreateOptions{FieldManager: FieldManager})
	}
	...
	// If the Deployment is not controlled by this TaskCluster resource, we should log
	...
	// If this number of the replicas on the TaskCluster resource is specified, and the
	...
	if taskCluster.Spec.Replicas != nil && *taskCluster.Spec.Replicas != *deployment.Spec.Replicas {
		...
		deployment, err = c.kubeclientset.AppsV1().Deployments(taskCluster.Namespace).Update(context.TODO(), newDeployment(taskCluster), metav1.UpdateOptions{FieldManager: FieldManager})
	}
	...
	// Finally, we update the status block of the TaskCluster resource to reflect the
...
func (c *TaskClusterController) updateTaskClusterStatus(taskCluster *samplev1alpha1.TaskCluster, deployment *appsv1.Deployment) error {
	...
	// we must use Update instead of UpdateStatus to update the Status block of the TaskCluster resource.
	// UpdateStatus will not allow changes to the Spec of the resource,
	// which is ideal for ensuring nothing other than resource status has been updated.
	_, err := c.sampleclientset.SamplecontrollerV1alpha1().TaskClusters(taskCluster.Namespace).UpdateStatus(context.TODO(), taskClusterCopy, metav1.UpdateOptions{FieldManager: FieldManager})
	return err
}
...
// to find the TaskCluster resource that 'owns' it. It does this by looking at the
...
func (c *TaskClusterController) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		// If this object is not owned by a TaskCluster, we should not do anything more
		// with it.
...
// newDeployment creates a new Deployment for a TaskCluster resource. It also sets
// the appropriate OwnerReferences on the resource so handleObject can discover
// the TaskCluster resource that 'owns' it.
func newDeployment(taskCluster *samplev1alpha1.TaskCluster) *appsv1.Deployment {
	labels := map[string]string{
		"app":        "nginx",
		"controller": taskCluster.Name,
	}
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(taskCluster, samplev1alpha1.SchemeGroupVersion.WithKind("TaskCluster")),
			},
		},
